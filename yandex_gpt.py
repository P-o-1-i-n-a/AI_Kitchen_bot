...
def build_prompt(user_data: dict) -> str:
    base_prompt = [
        ...
        "",
        "Требования:",
        "- Учет диетических ограничений, обратить внимание на запрещенные продукты. запрещенные в выбранной диете продукты запрещено брать в рецепт. стоит выдать краткое предупреждение пользователю об этом",
        "- Примерный расчет КБЖУ на порцию, указание колличества грамм на порцию",
        "- Четкая пошаговая инструкция"
        "- Полезные советы по приготовлению, сервировке или хранению блюда"
    ]
...
def clean_recipe_text(text: str) -> str:
    ...
    unwanted_phrases = [
        ...
        "В интернете есть много сайтов с информацией на эту тему. [Посмотрите, что нашлось в поиске](://.)",
