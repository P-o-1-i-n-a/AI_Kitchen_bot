# Глобальное хранилище состояний пользователей
# Формат: {chat_id: {step: str, meal_time: str, cuisine: str, diet_type: str, ingredients: str, allergies: str}}
user_states = {}

# Правила для различных диет
DIET_RULES = {
    "🚫 Нет ограничений": {
        "description": "Без диетических ограничений",
        "forbidden": [],
        "replacements": {}
    },
    "⚠️ Аллергии": {
        "description": "Исключены указанные аллергены",
        "forbidden": [],  # Заполняется динамически из user_states
        "replacements": {}
    },
    "⚖️ Низкокалорийные": {
        "description": "Менее 300 ккал на порцию",
        "forbidden": [
            "масло сливочное",
            "майонез",
            "сало",
            "сливки",
            "сахар"
        ],
        "replacements": {
            "масло сливочное": "оливковое масло (вдвое меньше)",
            "сливки": "обезжиренное молоко",
            "майонез": "греческий йогурт",
            "сахар": "стевия/эритритол"
        }
    },
    "💪 Высокобелковые": {
        "description": "Не менее 20г белка на порцию",
        "forbidden": [],
        "replacements": {
            "макароны": "нутовая паста",
            "рис": "киноа",
            "хлеб": "протеиновые хлебцы"
        }
    },
    "☪️ Халяль": {
        "description": "Соответствует исламским нормам",
        "forbidden": [
            "свинина",
            "бекон",
            "сало",
            "алкоголь",
            "желатин"
        ],
        "replacements": {
            "свинина": "говядина/баранина",
            "бекон": "индейка халяль",
            "алкоголь": "лимонный сок/уксус",
            "желатин": "агар-агар"
        }
    },
    "☦️ Постная": {
        "description": "Без продуктов животного происхождения",
        "forbidden": [
            "мясо",
            "курица",
            "рыба",
            "молоко",
            "сыр",
            "яйца",
            "сливочное масло"
        ],
        "replacements": {
            "мясо": "грибы/баклажаны",
            "молоко": "растительное молоко",
            "яйца": "льняная смесь (1 ст.л. льна + 3 ст.л. воды = 1 яйцо)",
            "сливочное масло": "растительное масло"
        }
    }
}

# Возможные шаги пользователя
USER_STEPS = {
    "waiting_meal_time": "ожидает выбора времени приема пищи",
    "waiting_cuisine": "ожидает выбора кухни",
    "waiting_diet": "ожидает выбора диеты",
    "waiting_allergies": "ожидает ввода аллергенов",
    "waiting_ingredients": "ожидает ввода ингредиентов",
    "generating_recipe": "генерирует рецепт"
}

def reset_user_state(chat_id: int):
    """Сбрасывает состояние пользователя"""
    user_states[chat_id] = {"step": "waiting_meal_time"}