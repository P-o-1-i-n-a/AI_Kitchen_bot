...
async def generate_recipe(chat_id: int):
    try:
        ...
        prompt = f"""Ты - профессиональный шеф-повар. Создай рецепт строго по следующим требованиям:
...
2. Строгие правила:
{'- Исключи следующие аллергены: ' + data['allergies'] if data['diet_type'] == "⚠️ Аллергии" else ''}
{"- " + "\n- ".join(f"Исключи {i}" for i in diet_info["forbidden"]) if diet_info["forbidden"] else ""}
{"- " + "\n- ".join(f"Замени {k} на {v}" for k, v in diet_info["replacements"].items()) if diet_info["replacements"] else ""}

3. Требования к формату:
🍽 Название блюда (укажи тип диеты, если не стандартный)
🌍 Кухня: {data['cuisine']}
🥗 Диета: {data['diet_type']} ({diet_description})
⚠️ Изменения: {"Нет" if not diet_info["forbidden"] else "Исключено: " + ", ".join(diet_info["forbidden"])}

⏱ Время приготовления (разбивка по этапам)
👨‍🍳 Порций: [число]

📋 Ингредиенты (на 1 порцию):
- [ингредиент] [кол-во] {"(замена)" если была замена}
...
- По подаче и хранению"""
        ...
        body = {
            ...
            "completionOptions": {
                "stream": False,
                "temperature": 0.5,  # Более строгий режим для соблюдения диет
                "maxTokens": 2500
            },
            "messages": [
                {
