...
async def generate_recipe(chat_id: int):
    try:
        ...
        if data['diet_type'] == "⚠️ Аллергии":
            ...
            diet_prompt = " Учитывай правила халяль.Строго исключи свинину и алкоголь. Все мясо должно быть халяльным. отправь небольшое сообщение с предупреждением если пользователь ввел ингридиенты, не подходящие под эту диету. и не бери в рецепт неподходящие для диеты ингридиенты"
        elif data['diet_type'] == "☦️ Постная":
            diet_prompt = " Учитывай постные правила. Исключи мясо, молочные продукты, яйца и рыбу. Используй растительные ингредиенты.. отправь небольшое сообщение с предупреждением если пользователь ввел ингридиенты, не подходящие под эту диету. и не бери в рецепт неподходящие для диеты ингридиенты"
