...
async def generate_recipe(chat_id: int):
    try:
        ...
        prompt = f"""Ты профессиональный шеф-повар. Сгенерируй рецепт строго по этим правилам:

1. Основные параметры:
- Тип: {data['meal_time']}
- Кухня: {data['cuisine']}
- Диета: {data['diet_type']} ({diet_description})
- Исходные ингредиенты: {data['ingredients']}

2. Строгие требования:
- Никогда не предлагай искать информацию в интернете
- Всегда придерживайся указанного формата
- Если ингредиенты не подходят для диеты - замени их
- Рассчитай точное КБЖУ для каждой порции

3. Формат ответа (обязателен):
```plaintext
🍽 [Название блюда] ({data['diet_type']} версия)

🌍 Кухня: {data['cuisine']}
🥗 Диета: {data['diet_type']} ({diet_description})
⏱ Время приготовления: [общее время] ([разбивка по этапам])
👨‍🍳 Количество порций: [число]

📋 Ингредиенты (на 1 порцию):
- [ингредиент] [количество]{" (замена)" если была замена}
[общее количество для всех порций в скобках]
...
...
```"""
        ...
        body = {
            ...
            "messages": [
                {
                    "role": "system",
                    "text": "Ты профессиональный шеф-повар. Всегда генерируй рецепт. Никогда не предлагай поискать в интернете. Строго соблюдай диетические ограничения и формат ответа."
                },
                {
                    "role": "user", 
                    "text": prompt
        ...
        async with httpx.AsyncClient(timeout=90.0) as client:
            ...
            if "интернет" in recipe.lower() or "поиск" in recipe.lower():
                raise ValueError("Недопустимое предложение поиска в интернете")
        ...
        await bot.send_message(chat_id, "⚠️ Ошибка при генерации рецепта. Пожалуйста, попробуйте еще раз.", reply_markup=main_keyboard())
